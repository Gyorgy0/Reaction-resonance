    ...
    for i in 0..row_count {
        for j in 0..col_count {
            ...
            if (cell.0.mass > 0.0){
                ...
                else if ((i as i32 - 1) - (row_count-1)) < cell.1.y as i32 && (!cell.2) && cell.1.y > 1.0 {
                    cell.1.y = (-1.0)* ((i as i32) - (row_count-1)) as f32;
                    let tmp: Particle =
                        game_board[(((i + cell.1.y as i32) * col_count) + j) as usize];
                    game_board[(((i + cell.1.y as i32) * col_count) + j) as usize] = cell;
                    game_board[(((i + cell.1.y as i32) * col_count) + j) as usize].1.y = 0.0;
                    game_board[(((i + cell.1.y as i32) * col_count) + j) as usize].2 = true;
                    game_board[((i * col_count) + j) as usize] = tmp;
                }
    ...
    if is_mouse_button_down(btn)
        {
            ...
            if (cursor_position.0 >= 10.0 && cursor_position.0 <= 510.0 && cursor_position.1 >= 10.0 && cursor_position.1 <= 510.0) 
            {
                let x = cursor_position.0 as u32 / CELLSIZE;
                let y = cursor_position.1 as u32 / CELLSIZE;
                game_board[(y * col_count as u32 + x) as usize] = Particle(WATER, vec2(0.0, 0.0), false);
            }
        }
